...
(function (window) {
    ...
    smarty.utils = {
        ...
        setlocale: function (category, locale) {
            ...
            // %          note 3: Consider using http://demo.icu-project.org/icu-bin/locexp as basis for localization
            // (as in i18n_loc_set_default()) *     example 1: setlocale('LC_ALL', 'en_US'); *     returns 1: 'en_US'
            ...
            // Function usable by a ngettext implementation (apparently not an accessible part of setlocale(), but
            // locale-specific) See http://www.gnu.org/software/gettext/manual/gettext.html#Plural-forms though amended
            // with others from https://developer.mozilla.org/En/Localization_and_Plurals (new categories noted with
            // "MDC" below, though not sure of whether there is a convention for the relative order of these newer
            // groups as far as ngettext) The function name indicates the number of plural forms (nplural) Need to look
            // into http://cldr.unicode.org/ (maybe future JavaScript); Dojo has some functions (under new BSD),
            ...
            if (!phpjs.locales) {
                ...
                phpjs.locales.en = {
                    'LC_COLLATE': // For strcoll
                        ...
                        function (str1, str2) { // Fix: This one taken from strcmp, but need for other locales; we don't use localeCompare since its locale is not
                                                // settable
                    ...
                    'LC_MONETARY': { // Based on Windows "english" (English_United States.1252) locale
                        ...
                        mon_thousands_sep: ',',
                        mon_grouping: [3],
                        // use mon_thousands_sep; "" for no grouping; additional array members indicate successive
                        // group lengths after first group (e.g., if to be 1,23,456, could be [3, 2])
                        positive_sign: '',
                        negative_sign: '-',
                        int_frac_digits: 2,
                        ...
                        p_sep_by_space: 0,
                        // 0: no space between curr. symbol and value; 1: space sep. them unless symb. and sign are
                        // adjacent then space sep. them from value; 2: space sep. sign and value unless symb. and sign
                        // are adjacent then space separates
                        n_cs_precedes: 1,
                        // see p_cs_precedes
                        n_sep_by_space: 0,
                        // see p_sep_by_space
                        p_sign_posn: 3,
                        // 0: parentheses surround quantity and curr. symbol; 1: sign precedes them; 2: sign follows
                        // them; 3: sign immed. precedes curr. symbol; 4: sign immed. succeeds curr. symbol
                ...
                phpjs.locales.C = _copy(phpjs.locales.en); // Assume C locale is like English (?) (We need C locale for
                                                           // LC_CTYPE)
            ...
            if (locale === '0' || locale === 0) {
                if (category === 'LC_ALL') {
                    for (categ in this.php_js.localeCategories) {
                        cats.push(categ + '=' + this.php_js.localeCategories[categ]); // Add ".UTF-8" or allow
                                                                                      // ".@latint", etc. to the end?
                    }
                    return cats.join(';');
                }
        ...
        strftime: function (fmt, timestamp) {
            ...
            var _formats = {
                ...
                j: function (d) {
                    ...
                    ms += d.getTimezoneOffset() * 60000; // Line differs from Yahoo implementation which would be
    ...
    smarty.Template.prototype = {
        ...
        _triggerIncludesHandler: function (includes) {
            ...
            //					throw new smarty.Exception("Template loading timeout is exceeded for the folowings
            // templates: '{0}'", includes); }, includesTimeout);
        },
        ...
        _caseVar: function (req) {
            ...
            cycle: while ((ch = this._next())) {
                switch (true) {
                    ...
                    //						this._error('Only "\w\d" characters are allowed to use in variable
                    // name!'); break; $
                    case (/\$/).test(ch):
